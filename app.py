...
# ── Frontend-safe payload formatter ─────────────────
def build_frontend_payload(result):
    river_data = result.get('river_data', [])
    rainfall_summary = result.get('rainfall_summary', {})
    total_rainfall = result.get('total_rainfall', 0)
    file_date = result.get('file_date') or str(date.today())
    ...
    for item in river_data:
        ...
        try:
            ...
            if not np.isnan(value):
                numeric_levels.append(value)
        except Exception:
            continue

    stats = {
        'total': round(float(total_rainfall or sum(numeric_levels) or 0), 2),
        'today_rain': round(float(total_rainfall or sum(numeric_levels) or 0), 2),
        'mean': round(float(np.mean(numeric_levels)) if numeric_levels else 0, 2),
        'max': round(float(np.max(numeric_levels)) if numeric_levels else 0, 2),
        'heavy_days': int(sum(1 for v in numeric_levels if v >= 64.5)),
        'very_heavy': int(sum(1 for v in numeric_levels if v >= 115.6)),
        'records': len(river_data),
        'std': round(float(np.std(numeric_levels)) if numeric_levels else 0, 2),
        'date_from': file_date,
        'date_to': file_date,
    }
    ...
    district = {}
    for item in river_data:
        dist = str(item.get('district') or 'Unknown').strip() or 'Unknown'
        if dist not in district:
            district[dist] = {'total': 0.0, 'mean': 0.0, 'max': 0.0, 'records': 0}
        level = item.get('level')
        try:
            level_value = float(level)
        except Exception:
            level_value = None
        if level_value is not None and not np.isnan(level_value):
            district[dist]['total'] += level_value
            district[dist]['max'] = max(district[dist]['max'], level_value)
        district[dist]['records'] += 1

    for dist_name, values in district.items():
        if values['records']:
            values['mean'] = round(values['total'] / values['records'], 2)
            values['total'] = round(values['total'], 2)
            values['max'] = round(values['max'], 2)
    ...
    for item in river_data:
        ...
        try:
            ...
            level_value = None
        if level_value is not None and not np.isnan(level_value):
            severity = 'Very Heavy' if level_value >= 115.6 else 'Heavy' if level_value >= 64.5 else None
            if severity:
                alerts.append({
                    'dist': str(item.get('district') or item.get('river') or 'Unknown').strip() or 'Unknown',
                    'date': file_date,
                    'rain': round(level_value, 2),
                    'level': severity,
                })

    recent = []
    for item in river_data[:8]:
        recent.append({
    ...
    return {
        ...
        'cumulative': {
            'dates': [file_date] if file_date else [],
            'cumsum': [stats['total']],
            'daily': [stats['today_rain']],
        },
...
def upload():
    try:
        ...
        return jsonify(build_frontend_payload(result))
